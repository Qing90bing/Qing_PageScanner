...
`;
  ...
  function populateModalContent(modalContent) {
    ...
    setOutputTextarea(virtualScrollContainer);
  ...
  function updateStatistics(lineCount = 0, charCount = 0) {
    if (!statsContainer) return;
    statsContainer.textContent = `${t("results.stats.lines")}: ${lineCount} | ${t("results.stats.chars")}: ${charCount}`;
  }
  function updateLineNumbers(lineCount = 0) {
    if (!lineNumbersDiv) return;
    const fragment = document.createDocumentFragment();
    const numToRender = Math.min(lineCount, 1e3);
    for (let i = 1; i <= numToRender; i++) {
      const div = document.createElement("div");
      div.textContent = i;
      fragment.appendChild(div);
    }
    lineNumbersDiv.replaceChildren(fragment);
    const sizer = lineNumbersDiv.querySelector(".ts-line-number-sizer");
    if (sizer) {
      sizer.style.height = `${lineCount * 20}px`;
    }
  }
  function initializeLineNumbers() {
    ...
    lineNumbersDiv.appendChild(sizer);
  }
  var worker2 = null;
  function performQuickScan(onResult) {
    if (worker2) {
      worker2.terminate();
    }
    try {
  ...
  function createMainModal({ clearSessionCallback }) {
    ...
    scroller = new VirtualScroller(outputTextarea, []);
...
/* --- From virtualScroll.css --- */
/* src/assets/styles/virtualScroll.css */
.ts-virtual-scroll-container {
    height: 100%;
    width: 100%;
    overflow-y: auto;
    position: relative;
...
.ts-virtual-scroll-sizer {
    position: relative;
    width: 100%;
    height: 0; /* This will be set by JS */
}
.ts-virtual-scroll-content {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
